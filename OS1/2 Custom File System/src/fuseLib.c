...
{
    NodeStruct *nodo = myFileSystem.nodes[idxNode];
    char block[BLOCK_SIZE_BYTES];
    int i, diff = newSize - nodo->fileSize;
    ...
    if(diff > 0) {
        ...
        if(nodo->numBlocks && nodo->fileSize % BLOCK_SIZE_BYTES) {
            int bloqueActual = nodo->blocks[nodo->numBlocks - 1];

            if( readBlock(&myFileSystem, bloqueActual, &block)==-1 ) {
                fprintf(stderr,"Error reading block in resizeNode\n");
                return -EIO;
            }

            int desplBloque = nodo->fileSize % BLOCK_SIZE_BYTES;
            int bytes2Write = (diff > (BLOCK_SIZE_BYTES - desplBloque)) ? BLOCK_SIZE_BYTES - desplBloque : diff;
            for(i = 0; i < bytes2Write; i++) {
                block[desplBloque++] = 0;
            }

            if( writeBlock(&myFileSystem, bloqueActual, &block)==-1 ) {
        ...
        int newBlocks = (newSize + BLOCK_SIZE_BYTES - 1) / BLOCK_SIZE_BYTES - nodo->numBlocks;
        if(newBlocks) {
            memset(block, 0, sizeof(char)*BLOCK_SIZE_BYTES);
            ...
            int bloqueActual = nodo->numBlocks;
            nodo->numBlocks += newBlocks;

            for(i = 0; bloqueActual != nodo->numBlocks; i++) {
                if(myFileSystem.bitMap[i] == 0) {
                    myFileSystem.bitMap[i] = 1;
                    nodo->blocks[bloqueActual] = i;
                    bloqueActual++;
    ...
    /// File decreases
        ...
        myFileSystem.superBlock.numOfFreeBlocks += (nodo->numBlocks - numBlocks);

        for(i = nodo->numBlocks; i > numBlocks; i--) {
            int nBloque = nodo->blocks[i - 1];
    ...
    updateNode(&myFileSystem, idxNode, nodo);
...
static int my_getattr(const char *path, struct stat *stbuf)
{
    ...
    if((idxDir = findFileByName(&myFileSystem, (char *)path + 1)) != -1) {
        nodo = myFileSystem.nodes[myFileSystem.directory.files[idxDir].nodeIdx];
        stbuf->st_size = nodo->fileSize;
        stbuf->st_mode = S_IFREG | 0644;
        stbuf->st_nlink = 1;
        stbuf->st_uid = getuid();
        stbuf->st_gid = getgid();
        stbuf->st_mtime = stbuf->st_ctime = nodo->modificationTime;
        return 0;
    }
...
static int my_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    while(bytes2Write) {
        int i;
        int bloqueActual, desplBloque;
        bloqueActual = nodo->blocks[offset / BLOCK_SIZE_BYTES];
        desplBloque = offset % BLOCK_SIZE_BYTES;

        if( readBlock(&myFileSystem, bloqueActual, &buffer)==-1 ) {
            fprintf(stderr,"Error reading blocks in my_write\n");
            return -EIO;
        }

        for(i = desplBloque; (i < BLOCK_SIZE_BYTES) && (totalWrite < size); i++) {
            buffer[i] = buf[totalWrite++];
        }

        if( writeBlock(&myFileSystem, bloqueActual, &buffer)==-1 ) {
            fprintf(stderr,"Error writing block in my_write\n");
            return -EIO;
        }

        // Discount the written stuff
        bytes2Write -= (i - desplBloque);
        offset += (i - desplBloque);
    }
    sync();
...
int my_unlink(const char *filename) {
	...
	char* name=(char*)(filename+1); 
		
	int nodeIndexDir = findFileByName(&myFileSystem, name);
    ...
    int nodeIdx = myFileSystem.directory.files[nodeIndexDir].nodeIdx; 
	NodeStruct* inode=myFileSystem.nodes[nodeIdx];
	
	myFileSystem.directory.numFiles -= 1; 
	myFileSystem.directory.files[nodeIndexDir].freeFile = true; // Nodo libre
...
int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
	char buffer[BLOCK_SIZE_BYTES];
	int bytesRestantes = size;
    int leidos=0;
	NodeStruct *nodo = myFileSystem.nodes[fi->fh]; // inodo asociado al fichero
	...
	while(bytesRestantes) {
		...
		if( readBlock(&myFileSystem, bloqueActual, &buffer)==-1 ) {
            fprintf(stderr,"Error leyendo en my_write\n");
            return -EIO;
        }

        int i = 0;

		for(i = desplBloque; (i < BLOCK_SIZE_BYTES) && (leidos < size); i++) {
            buf[leidos++] = buffer[i] ;
        }

        bytesRestantes -= (i - desplBloque);
        offset += (i - desplBloque);
